    ...
    protected void GenerateBody()
    {
        ...
        foreach (SegmentData currSegmentData in creatorData.spineSegmentData)
        {
            for (int i = 0; i < currSegmentData.jointCount; i++)
            {
                ...
                AnimalJoint newJoint = GenerateSegment(segmentData: currSegmentData, iteration: i, masterTransform, positionOffset, segmentScale, yOffset: 0f, creatorData.spineColor, name, spriteOffset: 0.0015f);
        ...
        foreach (AnimalLimbData currLimbData in creatorData.animalLimbData)
        {
            ...
            foreach (SegmentData currSegmentData in currLimbData.joints)
            {
                ...
                for (int i = 0; i < currSegmentData.jointCount; i++)
                {
                    ...
                    limbJoints.Add(GenerateSegment(currSegmentData, iteration: i, masterTransform, positionOffset, segmentScale, -0.002f - 0.0001f * limbId, currLimbData.limbColor, name, spriteOffset: 0.001f));
